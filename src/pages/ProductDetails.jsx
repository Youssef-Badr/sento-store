...
<div className="mb-6">
  ...
  <div
    ...
    onClick={() => setIsLightboxOpen(false)} // 👈 كليك واحد يقفل المودال
  >
    {/* 🔹 طبقة خفية للسحب فقط */}
    <div
      ...
      onTouchEnd={(e) => {
        ...
        if (Math.abs(diff) < 30) {
          // 👈 لو الفرق بسيط اعتبرها مجرد لمسة عادية
          setTouchStart(null);
          setTouchEnd(null);
          return;
        }
        ...
        // 👈 لو سحب حقيقي
        e.stopPropagation();
        if (diff > 50) nextLightbox(); // سحب لليسار
        else if (diff < -50) prevLightbox(); // سحب لليمين
